...
app.get('/api/playshare', async(req, res) => {
  try {
    const data = await client.query(
      'SELECT * from playshare where owner_id=$1', 
      [req.userId],
    );
...
// the client will hit this API with /DELETE /playshare/4
// because 4 lives in the URL, it is a req.params thing
app.delete('/api/playshare/:id', async(req, res) => {
  try {
    const data = await client.query(
      'DELETE from playshare where owner_id=$1 AND id=$2', 
